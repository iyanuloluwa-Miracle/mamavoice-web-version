        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              :placeholder="t('auth.passwordPlaceholder')"
              required
              class="w-full px-4 py-3 rounded-2xl border bg-mama-input text-mama-text placeholder-mama-muted/60 text-sm focus:outline-none focus:bg-mama-surface transition-colors"
              :class="passwordMismatch ? 'border-red-400' : 'border-mama-border focus:border-mama-teal'"
            />
            <p v-if="passwordMismatch" class="text-[11px] text-red-500 mt-1">{{ t('auth.passwordMismatch') }}</p>
          </div>
...
async function handleSubmit() {
  ...
  try {
    ...
    // Pass otpId and email via query string to verify page
    await navigateTo({
      path: '/auth/verify-email',
      query: { otpId: res.otpId, email: email.value },
    })
  } catch (err: unknown) {
