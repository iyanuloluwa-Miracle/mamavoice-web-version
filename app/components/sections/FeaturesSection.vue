<template>
  <section id="features" class="py-20 px-4 bg-mama-bg">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div
          ...
          :key="feature.title"
          class="bg-mama-surface rounded-3xl p-6 shadow-soft hover:shadow-soft-hover transition-all duration-300 group cursor-default"
          :style="{ animationDelay: `${i * 100}ms` }"
        >
          <!-- Icon -->
          <div
            class="w-14 h-14 rounded-2xl flex items-center justify-center mb-5 text-2xl transition-all duration-300 group-hover:scale-110"
            :style="{ backgroundColor: isDark ? feature.iconBgDark : feature.iconBg }"
          >
            {{ feature.emoji }}
          </div>
          <h3 class="text-lg font-bold text-mama-text mb-2">{{ feature.title }}</h3>
          ...
          <div class="mt-4">
            <span
              ...
              :style="{
                backgroundColor: isDark ? feature.tagBgDark : feature.tagBg,
